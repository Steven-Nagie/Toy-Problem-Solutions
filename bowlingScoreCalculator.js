...
var bowlingScore = function(rolls) {
  var score = 0;
  if (rolls.length === 12 && rolls[10] === 10 && rolls[11] === 10) {
    return 300;
  }

  // The problem with the way this is currently written is that any strike will screw up the i % 2 operation and throw off the scoring.
  // Perhaps the solution is a sort of flag, so we know which ball of each frame we are dealing with.
  for (var i = 0; i < rolls.length; i++) {
    if (rolls[i] === 10 && rolls[i + 3]) {
      score = score + rolls[i] + rolls[i+1] + rolls[i+2];
    } else if (i % 2 != 0 && rolls[i] + rolls[i+1] === 10 && rolls[i+2]) {
      score = score + rolls[i] + rolls[i+1] + rolls[i+2];
    } else {
      score += rolls[i];
    }
